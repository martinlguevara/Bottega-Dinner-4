...
// Función para determinar el horario de comida basado en la hora actual
function determinarHorarioComida(horaActual) {
    for (const horarioComida of horariosComida) {
        if (estaAbierto(horarioComida, horaActual)) {
            return horarioComida;
        }
    }
    return null; // Si no hay horarios de comida abiertos, se devuelve null
}
...
function principal() {
    try {
        ...
        const horarioComida = determinarHorarioComida(horaActual);

        if (horarioComida) {
            manejarSeleccionComida(horarioComida);
        } else {
            alert("El comedor está cerrado en este momento. Por favor, intenta en el horario adecuado.");
        }
    } catch (error) {
